...
from weiboScrapy import utils
from weiboScrapy.items import WeiboItem
from weiboScrapy.utils import load_cookies
...
class WeiboSearchSpider(scrapy.Spider):
    ...
    if not isinstance(keyword_list, list):
        ...
        keyword_list = utils.get_keyword_list(keyword_list)

    for i, keyword in enumerate(keyword_list):
        if len(keyword) > 2 and keyword[0] == '#' and keyword[-1] == '#':
            keyword_list[i] = '%23' + keyword[1:-1] + '%23'
    weibo_type = utils.convert_weibo_type(settings.get('WEIBO_TYPE'))
    contain_type = utils.convert_contain_type(settings.get('CONTAIN_TYPE'))
    regions = utils.get_regions(settings.get('REGION'))
    base_url = 'https://s.weibo.com'
    start_date = settings.get('START_DATE',
                              datetime.now().strftime('%Y-%m-%d'))
    end_date = settings.get('END_DATE', datetime.now().strftime('%Y-%m-%d'))
    if utils.str_to_time(start_date) > utils.str_to_time(end_date):
